...
The ``pages-meta-history*.xml-pNpM.7z`` files are the only published source
of per-revision wikitext on the WMF dump server. Unlike the gzip stub dumps
used by Phase 2, these are **7z-compressed**. This loader spawns the system
``7zz`` binary (``7zip`` package) per file with ``x -so`` and reads the
inner XML directly from its stdout — pure-Python ``py7zr`` decompression
was the wall-clock bottleneck (~4 rev/s/worker on the WMF dumps); ``7zz``
uses multi-threaded native LZMA2 and is 50-100x faster on the same files.
...
3. For each input ``.7z`` file: ``Popen(['7zz', 'x', '-so', path])`` and
   feed the stdout pipe directly into ``mwxml.Dump.from_file``. For each
   revision write ``(rev_id, rev_text bytes, rev_text_bytes)`` via psycopg's
   binary COPY into ``revision_text``.
...
DUMP_XML_DIR = Path("/media/simone/lacie14/wikidumps/20260401/xml")
META_HISTORY_GLOB = "enwiki-20260401-pages-meta-history*.7z"

PG_SERVICE = "service=wiki"
BATCH_SIZE = 5_000
PROGRESS_EVERY = 10_000  # fine-grained per-worker progress (was 100_000)
SEVENZ_BIN = "7zz"  # 7zip 21.07+; package: 7zip
SEVENZ_READ_BUFSIZE = 1 << 20  # 1 MiB stdout pipe buffer
...
class _SevenZReaper:
    """Wait for a `7zz` subprocess to exit and capture any failure mode.

    Replaces the queue/producer-thread plumbing of the prior py7zr-based
    implementation. The caller drains ``proc.stdout`` directly; this object
    owns the stderr drain and an outer ``join()`` that blocks until 7zz has
    fully terminated, populating ``errors`` on non-zero exit.
    """
    ...
    def __init__(self, proc: subprocess.Popen, path: Path,
        ...
        self._stderr_t = threading.Thread(
            ...
            name=f"7zz-stderr:{path.name}", daemon=True,
        )
    ...
    def join(self, timeout: Optional[float] = None) -> None:
        try:
            ...
            self._proc.kill()
            self._proc.wait()
        self._stderr_t.join(timeout=timeout)
        rc = self._proc.returncode
        if rc not in (0, -13):  # 0 = ok; -13 = SIGPIPE when consumer closed early
            err_blob = b"".join(self._stderr_chunks).decode(
                "utf-8", errors="replace")
            self._errors.append(RuntimeError(
                f"7zz exited rc={rc} for {self._path.name}: {err_blob[:500]}"
            ))


def open_7z_xml_stream(path: Path) -> tuple[io.IOBase, _SevenZReaper, list]:
    """Stream-decompress ``path`` via the system ``7zz`` binary.

    Spawns ``7zz x -so <path>`` and returns
    ``(text_fp, reaper, errors)``. The caller iterates
    ``mwxml.Dump.from_file(text_fp)`` directly off ``proc.stdout``; after
    consumption, ``reaper.join(timeout=...)`` blocks until 7zz exits and
    appends a RuntimeError to ``errors`` on non-zero rc.
    """
    errors: list[BaseException] = []
    proc = subprocess.Popen(
        [SEVENZ_BIN, "x", "-so", str(path)],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        bufsize=SEVENZ_READ_BUFSIZE,
    )
    reaper = _SevenZReaper(proc, path, errors)
    text_fp = io.TextIOWrapper(
        proc.stdout, encoding="utf-8", errors="surrogatepass")
    return text_fp, reaper, errors
...
def _open_xml_for_parse(
    path: Path,
) -> tuple[io.IOBase, Optional[object], Optional[list]]:
    """Open path as a text file-like for mwxml.

    Returns ``(fp, reaper_or_None, errors_or_None)``. ``reaper`` is a
    ``_SevenZReaper`` for .7z inputs (with ``.join(timeout=...)``), or None
    for plain/gzip inputs where no subprocess needs reaping.
    """
    if path.suffix == ".7z":
...
_STAGE_TABLE_DDL = """
    CREATE TEMP TABLE IF NOT EXISTS _rev_text_stage (
        rev_id         bigint,
        rev_text       bytea,
...
def ensure_stage_table(conn: psycopg.Connection) -> None:
    """Create the per-session TEMP staging table once.

    Idempotent (`CREATE TEMP TABLE IF NOT EXISTS`); called once at worker
    start-of-session before the first copy_batch.
    """
    with conn.cursor() as cur:
...
def copy_batch(conn: psycopg.Connection, rows: list[RevisionTextRow]) -> None:
    """COPY a batch of RevisionTextRows into ``revision_text`` with ON CONFLICT
    safety.

    The 20260401 enwiki ``pages-meta-history*.7z`` sub-files are not strictly
    disjoint by ``rev_id`` — at least one rev_id (e.g. 711706 for page 23639
    "Gasoline") appears across multiple sub-files. The same overlap killed
    the Phase 2 stub loader on 2026-05-20 (fix: commit `8ea1b1b`) and killed
    the Phase 3 loader on 2026-06-03 at row ~393 M. A naive
    ``COPY revision_text FROM STDIN`` therefore hits the PK constraint the
    moment any later worker's batch contains an already-inserted rev_id.

    Pattern below: COPY into a per-session TEMP table, then
    ``INSERT ... SELECT DISTINCT ON (rev_id) ... ON CONFLICT (rev_id) DO
    NOTHING`` — duplicates are silently skipped, throughput stays COPY-fast,
    and the loader becomes idempotent under both dump-overlap and arbitrary
    re-runs with ``--no-truncate``.
    """
    if not rows:
        return
    cols = ", ".join(COPY_COLUMNS)
    with conn.cursor() as cur:
        # Empty staging before refill. TRUNCATE on a TEMP table is
        # metadata-only (no WAL) and cheap.
        cur.execute("TRUNCATE _rev_text_stage")
        copy_sql = f"COPY _rev_text_stage ({cols}) FROM STDIN WITH (FORMAT BINARY)"
        with cur.copy(copy_sql) as cp:
            cp.set_types(["int8", "bytea", "int8"])
            for r in rows:
                cp.write_row((r.rev_id, r.rev_text, r.rev_text_bytes))
        # Drain staging into revision_text. DISTINCT ON guards intra-batch
        # duplicates (rare, possible at split boundaries); ON CONFLICT guards
        # cross-batch / cross-file / restart duplicates.
        cur.execute(
            f"INSERT INTO revision_text ({cols}) "
            f"SELECT DISTINCT ON (rev_id) {cols} FROM _rev_text_stage "
...
def load_one_file(
    ...
    with psycopg.connect(PG_SERVICE) as conn:
        # Per-session perf knobs (SET, not SET LOCAL — SET LOCAL would reset
        # on each commit and the next batch would lose the optimisation).
        with conn.cursor() as cur:
...
def _init_worker_logging(log_level: int) -> None:
    """ProcessPoolExecutor initializer: reset logging in the forked worker.

    Without this, child-process ``logger.info`` calls can be silently lost
    (handlers attached pre-fork aren't always re-armed on the child's
    inherited stderr fd). Reinstall a fresh stderr StreamHandler so
    per-file progress lines reach the parent's tee'd stdout/stderr.
    """
